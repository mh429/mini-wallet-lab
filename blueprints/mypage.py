...
def mypage():
    ...
    sqlUser = sqlUser + " WHERE id = '" + str(user_id) + "';" #WHERE等の予約文字の前には半角スペースをつける癖をつける
    print(sqlUser)
    ...
    sqlOrder = sqlOrder + " WHERE member_id = '" + str(user_id) + "' ORDER BY id DESC;" #WHERE等の予約文字の前には半角スペースをつける癖をつける
    print(sqlOrder)
    ...
    #画面を表示
    return render_template('mypage/mypage.html',user_info=user_info,order_history=order_history)
